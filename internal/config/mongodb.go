...
func InitializeCollections() {
	collections := []struct {
		...
		},
		{
			...
			schema: bson.M{
				"bsonType": "object",
				"required": []string{"mealType", "name", "image", "date", "userId", "macros"},
				"properties": bson.M{
					...
					"macros":   bson.M{"bsonType": "object", "description": "must be an object with macros for the meal"},
		...
		},
		{
			...
			schema: bson.M{
				"bsonType": "object",
				"required": []string{"name","calories", "protein", "carbs", "fat", "createdAt"},
				"properties": bson.M{
					"name":      bson.M{"bsonType": "string", "description": "must be a string"},
					"calories":  bson.M{"bsonType": "double", "description": "must be a double"},
					"protein":   bson.M{"bsonType": "double", "description": "must be a double"},
					"carbs":     bson.M{"bsonType": "double", "description": "must be a double"},
					"fat":       bson.M{"bsonType": "double", "description": "must be a double"},
					"createdAt": bson.M{"bsonType": "date", "description": "must be a valid date"},
				},
			},
		},
	}
